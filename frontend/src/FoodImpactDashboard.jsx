...
const presets = {
  ...
  all: () => ({ start: new Date(2000, 0, 1), end: new Date(2999, 11, 31), label: "All time" }),
};

const categoriesDefault = [
  "Fruits & Vegetables","Dairy & Eggs","Meat & Poultry","Grains & Bread",
  "Canned Goods","Frozen Foods","Beverages","Snacks","Other",
];
...
export default function FoodImpactDashboard({ user }) {
  ...
  useEffect(() => {
    ...
    const unsubInv = onSnapshot(invQ, (snap) => {
      ...
      setUsedItems(arr);
      // extend category list if new ones appear
    ...
    const unsubDon = onSnapshot(donQ, (snap) => {
      const arr = snap.docs.map((d) => {
        const x = d.data();
        const createdAt =
          (x.createdAt?.toDate?.() ? x.createdAt.toDate() : x.createdAt) || new Date();
        return {
          category: x.category || "Other",
          createdAt,
  ...
  const filteredUsed = useMemo(
    () => (categoryFilter === "all" ? usedItems : usedItems.filter(i => i.category === categoryFilter)),
    [usedItems, categoryFilter]
  );
  const filteredDon = useMemo(
    () => (categoryFilter === "all" ? donations : donations.filter(i => i.category === categoryFilter)),
  ...
  const savedPerDay = useMemo(() => {
    ...
    filteredUsed.forEach((i) => {
      ...
      if (map.has(key)) map.set(key, map.get(key) + (isFinite(i.quantity) ? i.quantity : 0));
    });
  ...
  const savedByCategory = useMemo(() => {
    ...
    filteredUsed.forEach((i) => {
      ...
      catMap.set(c, (catMap.get(c) || 0) + (isFinite(i.quantity) ? i.quantity : 0));
    });
  ...
  const totals = useMemo(() => {
    const totalSaved = filteredUsed.reduce((s, i) => s + (isFinite(i.quantity) ? i.quantity : 0), 0);
  ...
  const previousComparison = useMemo(() => {
    ...
    const prevDon = donations
      .filter((d) => inWindow(new Date(d.createdAt), prevStart, prevEnd))
      .length;
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          {["7d", "30d", "ytd", "all"].map((k) => (
            <button
              ...
              className={`px-3 py-1 rounded-md border ${
                rangeKey === k ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700"
              }`}
            >
              {k === "7d" ? "Last 7d" : k === "30d" ? "Last 30d" : k.toUpperCase()}
            </button>
          ))}
          <button
            onClick={() => setRangeKey("custom")}
            className={`px-3 py-1 rounded-md border ${
              rangeKey === "custom" ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700"
            }`}
          >
            Custom
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl shadow-sm p-4">
          ...
          <p className="text-3xl font-semibold">{totals.totalSaved.toFixed(0)} kg</p>
          <p className={`text-sm mt-1 ${previousComparison.savedPct >= 0 ? "text-green-600" : "text-red-600"}`}>
            {previousComparison.savedPct >= 0 ? "▲" : "▼"} {Math.abs(previousComparison.savedPct)}% vs prev period
          </p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-4">
          <p className="text-sm text-gray-500">Donations</p>
          <p className="text-3xl font-semibold">{totals.donationCount}</p>
          <p className={`text-sm mt-1 ${previousComparison.donPct >= 0 ? "text-green-600" : "text-red-600"}`}>
            {previousComparison.donPct >= 0 ? "▲" : "▼"} {Math.abs(previousComparison.donPct)}% vs prev period
          </p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-4">
          <p className="text-sm text-gray-500">Active Filters</p>
          <p className="text-base">{rangeKey === "custom"
            ? `${new Date(start).toLocaleDateString()} → ${new Date(end).toLocaleDateString()}`
            : Object.values(presets).includes(presets[rangeKey]) ? presets[rangeKey]().label : "Custom"}
          </p>
          <p className="text-sm text-gray-500">{categoryFilter === "all" ? "All categories" : categoryFilter}</p>
        </div>
      </div>
      ...
      {hasData ? (
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm p-4" style={{ height: 360 }}>
            <Line
              data={{
                labels: dayLabels,
                datasets: [
              ...
              options={{
                ...
                plugins: { legend: { position: "top" }, title: { display: true, text: "Food Saved Over Time" } },
          ...
          <div className="bg-white rounded-xl shadow-sm p-4" style={{ height: 360 }}>
            <Bar
              data={{
                labels: dayLabels,
                datasets: [{ label: "Donations", data: donationsPerDay, borderWidth: 1 }],
              }}
              options={{
                responsive: true,
                maintainAspectRatio: false,
                plugins: { legend: { position: "top" }, title: { display: true, text: "Donations Over Time" } },
                scales: { y: { beginAtZero: true, ticks: { precision: 0 } } },
              }}
            />
          </div>

          {/* Doughnut: saved by category */}
          <div className="bg-white rounded-xl shadow-sm p-4 grid md:grid-cols-2 gap-6 items-center">
            <div style={{ height: 320 }}>
              <Doughnut
                data={{
                  labels: savedByCategory.labels,
                  datasets: [{ data: savedByCategory.data }],
                }}
                options={{
                  responsive: true,
                  maintainAspectRatio: false,
                  plugins: { legend: { position: "right" }, title: { display: true, text: "Food Saved by Category" } },
                }}
              />
            </div>
            <div>
              ...
              <ul className="space-y-1 text-sm">
                {savedByCategory.labels
                  .map((label, idx) => ({ label, v: savedByCategory.data[idx] }))
                  .sort((a, b) => b.v - a.v)
                  .slice(0, 5)
                  .map((x) => (
                    <li key={x.label} className="flex justify-between">
                      <span>{x.label}</span>
                      <span className="font-medium">{x.v.toFixed(0)} kg</span>
                    </li>
                  ))}
              </ul>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-10 text-center">
          ...
          <p className="text-gray-600 mt-2">
            Start logging items and marking them as <strong>used</strong> or converting to
